...
To avoid duplication of columns across 2 tables, this plugin uses STI, so you'll need a "type" column on your table. It works even if your primary model is already using STI (in which case the "type" column is already there, ready to be used by this plugin).
...
To use this plugin, you must also install the "skip_stamp":http://github.com/openhood/skip_stamp plugin. If you plan to version your associations, you will also need "dirty_attributes":http://github.com/openhood/dirty_associations.

In your model, add these columns:
